...
/**
 * --strict-youdao：只要「有道」音源，有道取不到就记为失败，**不启用百度兜底**。
 * 2026-09-07 用户指示：「音频优先用有道的音频，没有有道音频的暂时不要生成」。
 */
const STRICT_YOUDAO = argv.includes("--strict-youdao");

/** 体积告警阈值：正常单词音频 5–30KB，有道偶发返回超长（含例句/异常录音） */
const BIG_FILE_BYTES = 60 * 1024;
const BIG_LOG = join(ROOT, "scripts", "audio_renai_big_files.log");
const bigFiles = [];
...
/** 下载后体积告警：有道偶发返回超长音频（实测 indoors 132KB / grassland 141KB） */
function checkBig(outPath, key, tag) {
  try {
    const sz = statSync(outPath).size;
    if (sz > BIG_FILE_BYTES) {
      bigFiles.push(`${tag}\t${key}\t${sz}`);
      console.log(`  ⚠️ 超长音频 ${(sz / 1024).toFixed(0)}KB：${key}`);
    }
  } catch {}
}
...
if (DO_US) {
  ...
  const { done, failed } = await runPool(tasks, async ({ entry, key, outPath }) => {
    try {
      ...
      checkBig(outPath, key, "us");
      return null;
    } catch (e1) {
      if (STRICT_YOUDAO) {
        console.log(`  失败（有道不可用，strict 模式不兜底）：${entry.id} ${entry.english} -> ${e1.message}`);
        return `有道失败 ${e1.message}`;
      }
      try {
...
if (bigFiles.length) {
  writeFileSync(BIG_LOG, bigFiles.join("\n") + "\n", "utf8");
  console.log(
    `\n⚠️ 超长音频（>${BIG_FILE_BYTES / 1024}KB）${bigFiles.length} 个，明细见 scripts/audio_renai_big_files.log`
  );
}
if (STRICT_YOUDAO) console.log("（--strict-youdao：有道失败未启用百度兜底）");
