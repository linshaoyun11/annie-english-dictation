#!/usr/bin/env node
/**
 * scripts/fill_waiyanshe_oxford_sl.mjs
 * 补录 waiyanshe 一线 SL + oxford 一线 SL 的音频
 *  - 从 .tmp-curr.mjs 读 waiyanshe + oxford 全线 unique
 *  - 同时下美音（type=2）+ 英音（type=1），有道失败用 Edge 兜底
 *  - 60KB 阈值，超过自动 Edge 重生
 */
...
try {
  ...
  const uniqAll = new Map();
  for (const line of ["waiyanshe", "oxford"]) {
    for (const u of m.CURRICULA[line]) {
      for (const e of u.entries) {
        const k = e.english.trim().toLowerCase();
        if (!uniqAll.has(k)) uniqAll.set(k, e.id);
      }
    }
  }
  ENTRIES = [...uniqAll.entries()].map(([text, id]) => ({ id, text }));
  ...
  for (const t of tasks) {
    ...
    try {
      ...
      if (buf.length > 60 * 1024) {
        console.log(`  • 有道 ${t.variant} huge(${buf.length}B) → Edge`);
        needRegen = true;
      ...
      console.log(`  • 有道 ${t.variant} 失败(${reason}) → Edge`);
      source = "edge";
      try {
