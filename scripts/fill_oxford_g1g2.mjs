#!/usr/bin/env node
/**
 * scripts/fill_oxford_g1g2.mjs
 * 补录 oxford 一线 G1G2 的音频（2026-09-08 重建后）
 *  - 从 .tmp-curr.mjs 读 oxford G1G2 unique text → id 列表
...
try {
  ...
  const ox = m.CURRICULA.oxford.filter((u) => u.grade <= 2);
  const uniq = new Map();
  for (const u of ox)
