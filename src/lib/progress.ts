...
export interface Progress {
   ...
   * "rj-g1"：人教版一年级起点线（SL 一线版真实教材 G1-G9）。
   * "wy-g1"：外研社一年级起点线（外研新标准一线版真实教材 G1-G9）。
   * 2026-09-09（v23）已废弃「3 年级起去重 1-2 年级已学词」机制，过滤逻辑不再执行；
   * lineTag 仍保留作为数据迁移的锚点，旧缓存加载时仍按「同 lineTag 才行」校验。
   * 缺失表示旧混合线数据，加载时自动迁移。
   */
  lineTag?: string;
}
