...
/** 按起点分组的教材清单（用户视角：先选起点，再选出版商）
 *  每组带 accent 主题色：组标题色点 + 左侧色条 + 整组 tint 背景，
 *  让三大类一眼可分（蓝=起步 / 紫=进阶 / 绿=初中）。 */
const CURRICULUM_GROUPS: {
  title: string;
  versions: CurriculumVersion[];
  accent: { bar: string; dot: string; tint: string; label: string };
}[] = [
  {
    title: "一年级起点",
    versions: ["renjiao", "waiyanshe", "oxford"],
    accent: {
      bar: "#3B82F6", // blue-500
      dot: "#3B82F6",
...
export default function SettingsPage({
  ...
  // 各版本统计（单元数 / 条目数）
  const stats = useMemo(() => {
    const allVersions = CURRICULUM_GROUPS.flatMap((g) => g.versions);
    ...
    <div className="h-full overflow-y-auto px-5 pb-10">
      ...
      <div className="mt-3 flex flex-col gap-4">
        {CURRICULUM_GROUPS.map((group) => (
          <section
            key={group.title}
            className="rounded-2xl border-l-[4px] pl-3 pr-1.5 py-3"
            style={{
              borderLeftColor: group.accent.bar,
              backgroundColor: group.accent.tint,
            }}
          >
            <h3 className="mb-2.5 flex items-center gap-2 px-1">
              <span
                className="inline-block h-2.5 w-2.5 rounded-full"
                style={{ backgroundColor: group.accent.dot }}
                aria-hidden
              />
              ...
              <span
                ...
                style={{
                  color: group.accent.bar,
                  backgroundColor: "rgba(255,255,255,0.6)",
                }}
              >
                {group.accent.label}
              </span>
              <span className="ml-auto text-[10px] font-normal text-text3">
