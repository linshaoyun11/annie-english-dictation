...
## 🔶 有道并非 100% 确定性（2026-09-09 新发现）
绝大多数文本两次下载字节一致，但**个别文本会返回不同录音版本**：
全量字节核查 9730 条中 2 条因此不匹配
（`visit one's grandparents` 美音：首次 36000B，复检时有道直接取不到；
 `be born with` 英音：首次 24000B，复检 17162B）。
⇒ 字节核查出现「不匹配」时**先别急着判词错**，可能是音源版本漂移；
  对这种条改用 **Edge 重生成**（确定性强，字节可复现）。

## 全量核查用「字节级」比 PCM 相关度更好
新库文件是生成时的原始字节、未二次转码，且音源确定性 ⇒
「重新取一次音源的字节 == 磁盘文件字节」即可 100% 断定正确，
比 PCM 相关度（阈值 0.90）严格得多，且不用 ffmpeg 解码，**约 9 条/秒**。
脚本：`scripts/verify_full_bytes.py`（import regen 的 sanitize/fname，防逻辑走样）。
最终：9730 条 = 有道 8614 + Edge 1116，**0 不匹配**。

## 工具（已提交 scripts/）
- `regen_audio_by_text.py` —— 按文本哈希命名全量重生成（**防错位核心**）
- `verify_full_bytes.py` —— 全量字节级核查（首选的日常核查手段）
...
## 体积账（2026-09-10 实测，用 git 历史 blob 精确统计）
| | 旧库 eac36b4 | 新库 | 变化 |
|---|---|---|---|
| 文件数 | 15829（美 8014 / 英 7815） | 9730（4865×2） | **−38.5%** |
| 体积 | 188.2 MB | 165.4 MB | **−12.1%** |
| 单文件均值 | 12.2 KB | 17.4 KB | **+43%** |

- **体积只降 12%，不是"小了一半"**。上一轮报的「412M → 194M」是新旧并存的
  临时状态，不是真实对比，别再引用。
- 降幅来源：① 按文本哈希命名后**同文本全局只存一份**（旧库按 entry.id 命名，
  同一词在 6 线中重复出现就各存一份）；② 清掉 **5159 个孤儿文件**
  （15829 − 10670，占 32.6%）—— 历史词库重建后 id 位移留下的垃圾。
- **新库没跑 `optimize_audio.py`**（旧库跑过），所以单文件反而大 43%
  （`name` 7436→10413 B、`listen` 9648→11565 B）。
  新库全是**真 MP3**（0 个 WAV 伪装），64–192 kbps / 48 kHz / 部分立体声。

### 压缩实测（2026-09-10，抽样 80 条，24 kHz / mono）
| 码率 | 全库预计 | 省 | 保真均值/最低 | <0.90 |
|---|---|---|---|---|
| 32k | 59.5 MB | −106.0 | 0.9821 / 0.8514 | 2 条 |
| **48k** | **88.1 MB** | **−77.4** | **0.9933 / 0.9463** | **0** |
| 64k | 116.7 MB | −48.8 | 0.9978 / 0.9779 | 0 |

48k 是 Edge TTS 默认规格，推荐档位。**用户 2026-09-10 决定暂不压缩**（先验证 78
的内容正确性），脚本已备好：`scripts/compress_library.py`（带逐文件保真校验，
不达标/省不下就自动保留原文件，全库约 20–50 分钟）+ `measure_compress_gain.py`（评估）。
要压时：`python scripts/compress_library.py --kbps 48`，完成后**必须升 APP_BUILD**，
且之后只能用 PCM 相关度抽查（字节级核查失效）。回滚：`git checkout -- public/audio`。

⚠️ **PCM 互相关判据：绝不能先 trim 去静音**。trim 后两段起点会差几十样本，
而负 lag 落在循环卷积数组尾部，只搜 `conv[:span]` 搜不到 ⇒ 算出 0.25 的假低分
（不去静音实测 0.99）。必须双向搜 `max(conv[:span].max(), conv[n-span:].max())`。
这是本项目里第 3 次栽在判据上（前两次：跨音源、小写 key）。

### ⚠️ 有道对「非词典词」会确定性逐字母拼读（2026-09-12，aah 事件）

- `aah`（外研一线 G1U1）美音被有道直接念字母 a-a-h。**复取字节与存量一致** ⇒
  源本身如此，不是错位。同源判据：存量 vs 有道"真词参照"相关 <0.1 ⇒ 非整读。
  英音同一 key 是整读 ⇒ 有道美英行为可不同，**两口音要分别验证**。
- 修复：仅换 defective 口音，Edge 重生成同名覆盖（Edge 对感叹词一律整读）。
- **全库拼读扫描三重判据**（`.workbuddy/tmp/scan_spellout2.py / scan_interj.py /
  gap_test.py`，2026-09-12 全库跑过：唯一缺陷= aah 美音）：
  ① 2-6 字母短词 ffmpeg 精确时长离群；② 存量/Edge 去静音时长比 >1.8；
  ③ 字母间隙检测（活跃段 >=2 且末段短 = 拼读特征）。
- **两类假阳性别误修**：有道尾部静音长短不一（时长长≠内容错）；
  全大写缩写（WWF/ABC/KM/RSVP…）字母拼读就是正确发音。
- 魔数校验：Edge 输出可能是 `ff f3`（MPEG2.5 L3）帧头，**只看前 2 字节**，
  别拿 3 字节精确比对（第 4 次栽在判据上）。

### ⚠️ 拼读问题第二轮：称谓词 + 三条扫描判据全部失效（2026-09-13，Ms 事件）

- `Ms`（外研一线/外研三起/仁爱多条）被读字母，用户实测实锤。**Edge 对 Ms/Dr
  的默认读法同样是逐字母**（与"miz"/"doctor"参照相关更高于整读参照）——
  aah 时"Edge 一律整读"的结论是错的，**别再信"换 Edge 就对"**。
- 已修 4 文件（同名覆盖，备份在 `.workbuddy/baselines/title-before-fix/`）：
  `ms` 美/英 → Edge 读 **"Miz"**（/mɪz/，Ms 的标准读音同音词）；
  `dr` 美、`dr.` 美 → Edge 读 **"doctor"**。修后同源相关 0.84~1.0 ✓。
  `mr`/`mrs` 双口音、`dr` 英音核验为整读未动。
- **为什么 9-12 的全库扫描没抓到**（三判据对 Ms 全失效，教训）：
  ① 正则 `^[a-z]{2,6}$` 扫的是 texts 清单纯小写——`ms` 其实在集合里，
  但 ② 时长比失效（拼读 "em-es" 连读可以比整读还短）；
  ③ 间隙检测失效（字母名连读无能量间隙，且爆破音/鼻音有假阳/假阴）。
  **同源互相关（字母拼接 vs 真词参照）也有模糊区 0.1~0.4**，cat=1.0/aah=0.03
  这种悬殊才可信。最终金标准 = 用户耳朵 + 同音词绕道生成（Miz）。
- 波及修复：仁爱线 **13 条带 `*` 星号词条**（教材标记被录进英文，含 `*Ms`），
  已去星号（id 不变、进度无影响），去星号词全部命中现有音频。

### 音频「跨教材线共享」实测（2026-09-11）
manifest 是**全局一个**（`public/audio/manifest.json` + `manifest-uk.json`），
key = 归一化文本、value = 文本哈希 ⇒ **六条线共用同一份 mp3 池**，与线、与 entry.id 都无关。
当前唯一文本 5885，manifest 6064 条（179 条无词条引用，属历史冗余，无害）。
各线复用率（该线文本中已被别的线覆盖的比例）：

| 线 | 文本 | 独有 | 复用 | 复用率 |
|---|---|---|---|---|
| renjiao(一线) | 2900 | 533 | 2367 | 82% |
| waiyanshe(一线) | 2161 | 306 | 1855 | 86% |
| oxford(一线) | 2018 | 503 | 1515 | 75% |
| renjiao3 | 3353 | 896 | 2457 | 73% |
| waiyanshe3 | 2072 | 277 | 1795 | 87% |
| renai | 2262 | 340 | 1922 | 85% |

三线两两交集：renjiao∩waiyanshe 1508、renjiao∩oxford 1221、waiyanshe∩oxford 1106。
**六线美/英音缺失均为 0**（脚本：`.workbuddy/tmp/build_texts.py` 重抽 + 比对 manifest）。
代价：**同形异读只能发一个音**（read/present/live/close 等按词性不同的读法）。
要修得把词性并入哈希键 ⇒ 共享失效、体积翻倍，**暂不做**。

### 缩写音频两类坑（2026-09-13 完整判定）

- **真词形大写缩写被按单词读**（与 Ms 反向）：US→us、WHO→who、IT→it、
  AM→am 已确诊修复（Edge 空格分字母重生成，build 104）。
- **⚠️ build 104 只修了美音，英音漏修**（2026-09-13 深夜用户复报后才抓到）：
  manifest 美/英指向同一文件 ID，英音走 `{id}-uk.mp3`——**任何音频修复必须
  美/英两份各验各修**（aah、Ms、缩写三案同规律）。英音审计出 8 个整词缺陷
  （US/WHO/IT/PM/USA/NBA/CBA/WWF），Edge Sonia 逗号分字母重生成（build 107），
  备份 `.workbuddy/baselines/acronym-uk-fix/`，审计/修复脚本在
  `.workbuddy/tmp/audit_uk_acronyms.py` / `fix_uk_acronyms.py`。
- **判据**（对 <0.6s 音频互相关 0.03~0.5 全模糊不可靠，用时长比）：
  有真词形 → 对单词参照时长比≈1 即确诊读词；无真词形 → 对 Edge 字母
  基准比值 0.85~1.3 视为正常拼字母，出界人工复核。
  ⚠️ 跨韵律互相关（逗号/空格分字母 vs 存量）即使同音源也全模糊（0.09~0.3），
  别用；build 104 的「corr=1.000」是同文本参照**循环验证**，证明不了读法。
  长文件可靠路径：时长比 + 整词参照时长对照（PM 1.13s=词参照 1.13s 这种分毫
  相合就是铁证）。
...
## 🔊 「听起来有噪音」类报障的排查法（2026-09-15 pear 事件建立）

用户报「某个词的音频有噪音」。这类报障**不是错位/跑飞**，判据完全不同 ——
上面的「时长/声段/互相关」全用不上，必须换一套。

### 第一步永远是「确认音源」，不要先怀疑文件坏了

- **格式指纹 = 音源指纹**：24000Hz mono = Edge 或豆包；44100Hz = 有道英；48000Hz = 有道美。
- **最强的一招：重新合成一次比对字节**。
  Edge 是**确定性**的（同 text+voice 逐字节可复现），有道也基本是。
  ⇒ 若存库文件与重新合成**逐字节相同**，则「文件被污染 / 与文本错位」可一次性排除，
  问题只能在**音源本身**。pear 就是这么排除的（md5 相同）。
- 顺便查 `git log --oneline -1 -- <path>`：只有一个提交 ⇒ 后续批次没动过它。

### 判据表（三个都会互相矛盾，别只信一个）

| 判据 | 含义 | 会被什么混淆 |
|---|---|---|
| **HNR 谐噪比** | 帧自相关基频峰 → `10log10(r/(1-r))` | **音素**：摩擦音词（six/fox/shh）天然极低；跨音源比无意义 |
| **谱平坦度 flat_med** | 逐帧 几何均值/算术均值，取中位 | 音素（摩擦音）；短帧/爆破音 |
| **4–8kHz 地毯 carpet** | 最响 35% 帧的平均谱，4–8k 相对 1kHz 峰 | **爆破音短帧**（会把 get/hat 算得比真噪的还差） |
| **响度（活跃 RMS）** | 见 `measure_loudness.py` | 无（但它解释「为什么偏偏这个词听着噪」） |
| **频谱图目视** ⭐ | 波形 + STFT 图，**动态范围裁到 −75dB** | 需要人看；**信息量最大** |

**⭐ 频谱图是本类问题的定案手段。** 关键技巧：把 vmin 抬到 **−75dB**（默认 −120 会把
噪底压黑，看不出任何东西）。宽带噪声表现为**主共振峰之上铺满均匀雾层**，
干净录音该区域是深色。`plot_zoom.py` 已实现（PIL 自绘，无需 matplotlib）。

### ⚠️ 必须做「同音素 + 同音源」控制，否则结论会完全反过来

pear 事件里我先拿 pear(Edge) 跟同单元的 apple/banana/orange(有道) 比，
得出「Edge 天生比有道噪 15dB」——**完全错**。做同音素控制（只取不含摩擦音、
≤12 字符，n=60/组）后真实关系是：

| 音源 | HNR 中位 | 4–8kHz 地毯中位 |
|---|---|---|
| Edge/豆包 (24k) | 5.97 | **−41.3 dB（最干净）** |
| 有道英 (44.1k) | 5.83 | −33.8 dB |
| 有道美 (48k) | 6.42 | −32.4 dB |

⇒ **整体上 Edge 比有道干净 8dB**。样本恰好选到了安静的录音 ⇒ 结论整个倒过来。
脚本 `controlled_compare.py`。

### 定案判据组合（三者一致才下结论）

**频谱图目视 + HNR + 地毯 + 响度**。不一致就老实说无法定案，别硬凑。

### 顺带发现（2026-09-15）

- **build 77 时代那批 Edge 文件从未做过响度归一**，整体比基线低 4~5dB
  （pear −4.96、get −3.92、hat −4.23、vet −4.33 dB）。build 111 只归一了豆包批次。
  全库英音响度分布：中位 −15.50dBFS，±2dB 内 75.6%、低 2~4dB 14.3%、
  **低 4dB 以上 5.6%**、高 2dB 以上 4.5%。要不要批量归一**待用户决定**。
- **有道英音经常返回 48kHz WAV（`pcm_s16le`）**，体积大（pear 108,622B）⇒ 当年被
  「>60KB 改用 Edge」规则退回。**但其实内容可能只有单个词**——先用
  `block_scan.py` 看声段（pear 只有 300–810ms 一段），**确认是单词就裁出来用**，
  比退 Edge 强。⇒ 该规则应改为「**先裁静音再判体积**」，不是直接退。

### 本轮新增工具（`.workbuddy/tmp/`，均未提交）

| 脚本 | 用途 |
|---|---|
| `noise_diag.py` | 噪底 / 削波 / 直流偏置 / 高频嘶声 / 声段 |
| `noise_metrics.py` | 谱平坦度、各频带占比、帧间突变 |
| `hnr.py` | HNR 谐噪比（听觉「发噪」的金标准之一） |
| `controlled_compare.py` | **同音素控制**下的音源对比（关键，别跳过） |
| `plot_noise2.py` / `plot_zoom.py` | PIL 自绘波形+频谱（无 matplotlib）；后者支持窄动态范围 |
| `noise_sweep.py` / `hnr_sweep.py` / `loud_sweep.py` | 全库扫描，8 线程 ≈1 分钟 |
| `block_scan.py` | 逐 5ms 块打印能量+过零率，找离散异响/尾部残留/确定裁剪点 |
| `fix_pear_uk.py` | 裁剪 + 响度对齐 + 编码（带 `--dry-run`；直接写目标，不用 tmp+move） |
| `gen_pear_preview.py` | 生成 A/B 试听页（base64 内嵌音频 + 频谱图 + 指标表） |
