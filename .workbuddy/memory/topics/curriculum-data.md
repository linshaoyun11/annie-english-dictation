...
## 🆕 一年级起点三线 2026-09-11 第三次重录（v25）

renjiao / waiyanshe / oxford 三条**一线**已全部换成按官方教材逐册录入的数据，
`grade` 直接是真实年级 1–9（不再有「册号→年级」归并）：

| 线 | 文件 | 单元 | 词条 | 覆盖 |
|---|---|---|---|---|
| renjiao（人教新起点） | `renjiaoSL.ts` | 124 | 3839 | 小学新起点 + 初中新目标，全 1–9 |
| waiyanshe（外研一起） | `waiyansheSL.ts` | 175 | 2892 | 小学 Module 级 + 初中（七上新教材 / 八九年级旧版并存） |
| oxford（沪教牛津） | `oxfordSL.ts` | 108 | 2501 | 小学 + 初中；**缺六年级下**（站点无词表） |

- `mergeBooksToGrades` / `dedupeEarlyGrades` **已删除**：真实教材有螺旋复现词，
  旧的 G1G2 去重会删掉真内容。
- **CURRICULUM_VERSION = 25**（2026-09-11 起）。
- 查证链与踩坑：**见 `.workbuddy/memory/2026-09-11.md`**、skill
  `annie-rebuild-curriculum` 的「A0 · 用户没有照片时」与「数据生成脚本」两节。
  （dzkbw 官方目录 + 51jiaoxi 分册词表；dzkbw **只有小学册**，初中 503。）
...
> 由 `MEMORY.md` 精简时移入（2026-09-25），原为「用户协作偏好」下的两条。

- **教材数据以用户提供的截图为唯一准绳**；第三方来源只用于查漏，**有出入时不改数据**
  （2026-09-05 再次强调：G4 不要找官方目录，按拍的来）。
- **逐册拍截图推进，一次一册**；每册固定 4 组页：
  **CONTENTS** + **Vocabulary in Each Unit** + **Vocabulary A-Z** + **Useful Expressions**，
  约 **22-25 张**/册。
- 拍照清单页面：`.workbuddy/preview/screenshot-checklist.html`。
