...
- ⚠️ **但定关键词前必须先核实副标题的「实际值」**：线上副标题实为
  ...
  副标题含「人教版 / 外研社」才把这两个词删掉的。
  ✅ **2026-09-25 用户截图证实缺口存在**（关键词实填 87 字符版、输入框显示剩余 13 ——
  第四次印证按字符计）：**「人教版 / 外研社」在名称 / 副标题 / 关键词三处全部缺席**，
  而它们覆盖 6 条教材线里的 **4 条**（人教一/三起点、外研社一/三起点）。
  ⇒ **副标题与关键词必须同一次定完**。**v1.0.1 方案（字符数已实测，可直接复制）**：
  副标题 → `人教版外研社教材同步 · 单词拼写练习`（19/30）；
  关键词 → 末尾追加 `,外研版`（**91/100，一个字都不用删** ——
  曾写"需挤掉等长的低价值词"是**没核实剩余额度就下的错误结论**，已作废）。
  详见 `docs/appstore-aso-copy.md` §5.3。
