...
| 可直接改（**App 级**） | 必须发新版本（**版本级**） |
|---|---|
| 副标题、名称、类目、内容分级、隐私政策 URL | 描述、关键词、截图、App 预览、营销 URL、What's New |
| **推广文本**（170 字符，连审核都不用） | App 图标（打包在二进制里，要出新包） |
...
> 2. **线上副标题为空** —— 第二高权重的搜索字段白放着。它是 App 级字段，
>    **可直接补、不用发版本**，属于最高性价比的一次改动。
